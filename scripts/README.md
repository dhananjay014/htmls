...
`visual-audit.mjs` automatically discovers every top-level guide, including the legacy Feature Interactions page. It opens every navigation view in headless Chrome, waits for Mermaid and MathJax, rejects visible raw LaTeX delimiters, checks desktop and 390px mobile overflow, smoke-tests interactive controls, and saves representative screenshots.
...
MathJax and Mermaid are vendored under `assets/`, so guide rendering and the visual audit work offline.
